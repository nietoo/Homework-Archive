...
class StochasticDescent(BaseDescent):
    ...
    def calc_gradient(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        return 2 * np.dot(X.iloc[sample].T, np.dot(X.iloc[sample], self.w) - y.iloc[sample]) / self.batch_size
...
class Adagrad(BaseDescent):
    ...
    def update_weights(self, gradient: np.ndarray, iteration: int) -> np.ndarray:
        ...
        """
        self.g += (gradient ** 2)
        delta = (self.eta(iteration) * gradient) / np.sqrt(self.eps + self.g)
        self.w -= delta
        return delta
...
class LinearRegression:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> LinearRegression:
        ...
        for i in range(self.max_iter):
            self.calc_loss(X, y)
#             print(i, self.loss_history[i])
            if np.linalg.norm(self.descent.step(X, y, i), 2) < self.tolerance:
                self.break_iter = i
                break
        return self
    ...
    def calc_loss(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        :param y: objects' target
        """
        mse = (np.square(self.predict(X) - y)).mean()
